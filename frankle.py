...
class Conv2(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Sequential(
            builder.conv1x1(64 * 4, 256),
            nn.ReLU(),
            builder.conv1x1(256, 256),
            nn.ReLU(),
            builder.conv1x1(256, 1),
        )

    def forward(self, x):
        out = self.convs(x)
        out = out.view(out.size(0), 64 * 4, 1, 1)
        out = self.linear(out)
        return out.squeeze()
...
class FC_adv(nn.Module):
    def __init__(self):
        super(FC_adv, self).__init__()
        builder = get_builder()
        self.linear = nn.Sequential(
            builder.conv1x1(10,10, first_layer=True),
            nn.ReLU(),
            builder.conv1x1(10, 2),
            #nn.ReLU(),
            #builder.conv1x1(10, 1),
        )

    def forward(self, x):
        #print(x.shape)
        out = x.view(x.size(0), 10, 1, 1)
        out = self.linear(out)
        return out.squeeze()

class FC_adv_celeba(nn.Module):
    def __init__(self):
        super(FC_adv_celeba, self).__init__()
        builder = get_builder()
        self.linear = nn.Sequential(
            builder.conv1x1(2,2, first_layer=True),
            nn.ReLU(),
            builder.conv1x1(2, 2),
            #nn.ReLU(),
            #builder.conv1x1(10, 1),
