        ...
        for i in reversed(self.filename):
            ...
            if data_count[s, l] <= min_cnt:
                if self.split =='train':
                    ...
                    if self.split == 'test' and  data_count[s,l] <= test_cnt:
                        tmp.append(i)
                    elif self.split == 'val' and data_count[s,l] > test_cnt:
                        tmp.append(i)
...
class UTKFace_X:
    def __init__(self,args):
        ...
        self.train_dataset = UTKFaceDataset(root=root, split = 'train', transform=train_transform)
        self.train_loader = torch.utils.data.DataLoader(self.train_dataset, batch_size=args.batch_size, shuffle=True, **kwargs)
        self.val_dataset = UTKFaceDataset(root=root, split = 'val', transform=test_transform)
        self.val_loader = torch.utils.data.DataLoader(self.val_dataset, batch_size=args.batch_size, shuffle=False, **kwargs)
        self.test_dataset = UTKFaceDataset(root=root, split = 'test', transform=test_transform)
        self.test_loader = torch.utils.data.DataLoader(self.test_dataset, batch_size=args.batch_size, shuffle=False, **kwargs)

        print(self.train_dataset.num_data)
        print(self.val_dataset.num_data)
        print(self.test_dataset.num_data)
