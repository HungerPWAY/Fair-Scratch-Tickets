...
def save_checkpoint(state, is_best, is_last, filename="checkpoint.pth", save=False):
    ...
    if is_last:
        shutil.copyfile(filename, str(filename.parent / "model_last.pth"))
...
def init_subnet_model_weight_with_score(model, prune_rate):
    print(f"==> Initialize model weights with score based on pruning rate {prune_rate}")

    for n, m in model.named_modules():
        if hasattr(m, "init_subnet_weight_with_score"):
            m.init_subnet_weight_with_score(prune_rate)
