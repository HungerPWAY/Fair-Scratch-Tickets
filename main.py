...
def main_worker(args):
    ...
    if args.task == 'ft_full':
        ...
        raise ValueError('The task is beyond our implementation!')
            ...
            #print(param)
    if args.fair_type is None:
        return ValueError('Fair Task is not given!')


    optimizer = get_optimizer(args, model)
    data = get_dataset(args)
    print("Train_Nmatrix",data.train_Nmatrix)
    print("Test_Nmatrix",data.test_Nmatrix)
    if args.lr_policy != "ReduceLROnPlateau":
        lr_policy = get_policy(args.lr_policy)(optimizer, args)
    else:
        lr_policy = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', verbose=True)
    ...
    if args.automatic_resume:
        ...
        if os.path.isfile(args.resume):
            best_acc1, best_unfairness = resume(args, model, optimizer)
        else:
    ...
    # Data loading code
    if args.evaluate:
        acc1, unfairness, test_loss = validate(
            data.test_loader, model, criterion, args, writer=None, epoch=args.start_epoch
        )
        log.info('Acc@1: %.2f, Unfairness: %.2f', acc1, unfairness)
        ...
        Test_write_result_to_csv(
            ...
            fair_regularization = args.fair_regularization
        )
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_acc1, train_unfairness, train_loss = train(
        ...
        if epoch % args.val_every == 0 or epoch == args.epochs - 1: 
            ...
            acc1, unfairness, val_loss = validate(data.val_loader, model, criterion, args, writer, epoch)
            validation_time.update((time.time() - start_validation) / 60)
            if args.lr_policy == "ReduceLROnPlateau":
                lr_policy.step(val_loss)
        ...
        writer.add_scalar("val/lr", cur_lr, epoch)
        end_epoch = time.time()
    ...
    ## Test process
    args.Nmatrix = data.test_Nmatrix
    model.load_state_dict(torch.load(ckpt_base_dir/'model_best.pth')["state_dict"])
    test_acc1, test_unfairness, test_loss = validate(
            data.test_loader, model, criterion, args, writer=None, epoch=args.start_epoch
        )
...
def get_directories(args):
    ...
    if args.log_dir is None:
        run_base_dir = pathlib.Path(
            f"runs/{config}/{args.name}/prune_rate={args.prune_rate}_fair_type={args.fair_type}_fair_regularization={args.fair_regularization}_lam_fair={args.lam_fair}"
        )
    else:
        run_base_dir = pathlib.Path(
            f"runs/{config}/{args.name}/prune_rate={args.prune_rate}_fair_type={args.fair_type}_fair_regularization={args.fair_regularization}_lam_fair={args.lam_fair}"
